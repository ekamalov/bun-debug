...
const CORS = {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, x-open-runtimes-secret',
};
...
Bun.serve({
    ...
    async fetch(request) {
        ...
        if (
            ...
            (request.headers.get('x-open-runtimes-secret') ?? '') !== (Bun.env['OPEN_RUNTIMES_SECRET'] ?? '')
        ) {
        ...
        Array.from(request.headers.keys())
            .filter((header: any) => !header.toLowerCase().startsWith('x-open-runtimes-'))
            .forEach((header: any) => {
        ...
        const url = `${scheme}://${host}${port.toString() === defaultPort ? '' : `:${port}`}${path}${
